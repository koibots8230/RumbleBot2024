...
public class Drivetrain extends SubsystemBase implements Logged {
  ...
  @Log SwerveModuleState[] stateSetpoint;
  @Log SwerveModuleState[] stateReal;
  ...
  public Drivetrain(boolean isReal) {
    ...
    swerveModules =
        new SwerveModule[] { // FL-FR-BL-BR
          new SwerveModule(
              ...
              DrivetrainConstants.DeviceIDs.FRONT_LEFT_TURN,
              0),
          new SwerveModule(
              DrivetrainConstants.DeviceIDs.FRONT_RIGHT_DRIVE,
              DrivetrainConstants.DeviceIDs.FRONT_RIGHT_TURN,
              1),
          new SwerveModule(
              DrivetrainConstants.DeviceIDs.BACK_LEFT_DRIVE,
              DrivetrainConstants.DeviceIDs.BACK_LEFT_TURN,
              2),
          new SwerveModule(
              DrivetrainConstants.DeviceIDs.BACK_RIGHT_DRIVE,
              DrivetrainConstants.DeviceIDs.BACK_RIGHT_TURN,
              3),
        };
    ...
    try (Notifier odometryUpdater =
      ...
      //            odometryUpdater.startPeriodic(1.0 / 200); // Run at 200hz //TODO: FOR THE BETTER
      // ODOMETRY
      odometryUpdater.startPeriodic(
          1.0 / 50); // Run at 200hz //TODO: FOR 50hz ODOMETRY (i think its default)
    }

    xController =
        new PIDController(
            isReal
                ? DrivetrainConstants.VX_CONTROLLER_REAL.kp
                : DrivetrainConstants.VX_CONTROLLER_SIM.kp,
            isReal
                ? DrivetrainConstants.VX_CONTROLLER_REAL.ki
                : DrivetrainConstants.VX_CONTROLLER_SIM.ki,
            isReal
                ? DrivetrainConstants.VX_CONTROLLER_REAL.kd
                : DrivetrainConstants.VX_CONTROLLER_SIM.kd);
    yController =
        new PIDController(
            isReal
                ? DrivetrainConstants.VY_CONTROLLER_REAL.kp
                : DrivetrainConstants.VY_CONTROLLER_SIM.kp,
            isReal
                ? DrivetrainConstants.VY_CONTROLLER_REAL.ki
                : DrivetrainConstants.VY_CONTROLLER_SIM.ki,
            isReal
                ? DrivetrainConstants.VY_CONTROLLER_REAL.kd
                : DrivetrainConstants.VY_CONTROLLER_SIM.kd);
    thetaController =
        new PIDController(
            isReal
                ? DrivetrainConstants.VTHETA_CONTROLLER_REAL.kp
                : DrivetrainConstants.VTHETA_CONTROLLER_SIM.kp,
            isReal
                ? DrivetrainConstants.VTHETA_CONTROLLER_REAL.ki
                : DrivetrainConstants.VTHETA_CONTROLLER_SIM.ki,
            isReal
    ...
    public SwerveModule(int driveId, int turnId, int index) {
      ...
      driveFeedforward =
          new SimpleMotorFeedforward(
              isReal
                  ? DrivetrainConstants.DRIVE_FEEDFORWARD_REAL.ks
                  : DrivetrainConstants.DRIVE_FEEDFORWARD_SIM.ks,
              isReal
                  ? DrivetrainConstants.DRIVE_FEEDFORWARD_REAL.kv
                  : DrivetrainConstants.DRIVE_FEEDFORWARD_SIM.kv,
              isReal
                  ? DrivetrainConstants.DRIVE_FEEDFORWARD_REAL.ka
                  : DrivetrainConstants.DRIVE_FEEDFORWARD_SIM.ka);
      driveFeedback =
          new PIDController(
              isReal
                  ? DrivetrainConstants.DRIVE_PID_CONSTANTS_REAL.kp
                  : DrivetrainConstants.DRIVE_PID_CONSTANTS_SIM.kp,
              isReal
                  ? DrivetrainConstants.DRIVE_PID_CONSTANTS_REAL.ki
                  : DrivetrainConstants.DRIVE_PID_CONSTANTS_SIM.ki,
              isReal
                  ? DrivetrainConstants.DRIVE_PID_CONSTANTS_REAL.kd
                  : DrivetrainConstants.DRIVE_PID_CONSTANTS_SIM.kd);
      turnFeedback =
          new PIDController(
              isReal
                  ? DrivetrainConstants.TURN_PID_CONSTANTS_REAL.kp
                  : DrivetrainConstants.TURN_PID_CONSTANTS_SIM.kp,
              isReal
                  ? DrivetrainConstants.TURN_PID_CONSTANTS_REAL.ki
                  : DrivetrainConstants.TURN_PID_CONSTANTS_SIM.ki,
              isReal
                  ? DrivetrainConstants.TURN_PID_CONSTANTS_REAL.kd
                  : DrivetrainConstants.TURN_PID_CONSTANTS_SIM.kd);
      ...
      if (turnId == DrivetrainConstants.DeviceIDs.FRONT_LEFT_TURN) {
        ...
        chassisAngularOffset =
            Rotation2d.fromRadians(Math.PI / 2); // Rotation2d.fromDegrees((3 * Math.PI) / 2);
      }
